...
class MyRoom extends colyseus_1.Room {
    onCreate(options) {
        ...
        this.setUp();
        //获取用户信息
        this.onMessage("getPlayerInfo", (client, message) => __awaiter(this, void 0, void 0, function* () {
            const result = yield db_1.getPlayer(this.state.users.get(client.sessionId));
            if (result.length == 0) {
        ...
        //保存场景信息
        this.onMessage("getSenceInfo", (client, message) => {
            //用户加入到场景中
            client.send("senceInfo", this.state.peopleNumber);
        });
        this.onMessage("toFloorOne", (client, message) => {
        ...
        this.onMessage("toFloorTwo", (client, message) => {
        ...
        this.onMessage("toFloorThree", (client, message) => {
            //用户进入三楼
            var isExit = this.state.playerInFloor.get(message);
            if (isExit == 1) {
                //获取值所在的索引值
        ...
        this.onMessage("toFloorFour", (client, message) => {
            //用户进入四楼
            //用户进入三楼
            var isExit = this.state.playerInFloor.get(message);
            if (isExit == 1) {
        ...
        //离开只需要一个就行
        this.onMessage("levelFloor", (client, message) => {
            //这里的离开指的是直接离开场景
            //删除元素在总人数数组
        ...
        this.onMessage("toabc", (client, message) => {
            console.log("收到请求信息");
            var a = 10;
            client.send("abc", a);
        });
        this.onMessage("postPlayerInfo", (client, message) => {
            //获取userId
    ...
    onJoin(client, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            //判断当前的玩家信息
            var isExit = yield db_1.getPlayer(userId);
            if (isExit.length == 0) {
                //玩家没有在数据库
                console.log('玩家不在数据库');
                var player = new MyRoomState_1.Player();
                ...
                //玩家数据添加到数据库
                let result = yield db_1.setPlayer(player);
                //添加到状态表中
